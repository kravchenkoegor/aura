...
class PlaywrightScraperService:
    ...
    async def scrape_instagram_post(
        ...
        try:
            ...
            logger.info(
                f"Successfully scraped {len(images_data)} images from {shortcode}"
            )
            ...
            for idx, img_element in enumerate(img_elements):
                try:
                    ...
                    if width < 100 or height < 100:
                        logger.debug(
                            f"Skipping small image {idx}: {width}x{height}"
                        )
                        continue
                    ...
                    images.append(image)
                    logger.info(
                        f"Extracted image {idx}: {width}x{height} from {img_url[:100]}"
                    )
                ...
                try:
                    ...
                    for element in caption_elements:
                        ...
                        if text and len(text.strip()) > 10:  # Reasonable caption length
                            logger.info(
                                f"Extracted description: {text[:100]}..."
                            )
                            return text.strip()
                except Exception:
                    continue
...
async def scrape_instagram_post_with_playwright(
    url: str, shortcode: str
) -> dict:
    """
